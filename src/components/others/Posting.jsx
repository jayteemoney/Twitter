...
>
  ...
  {showRetweetPopup === post.id && (
    <div 
      className="absolute bg-gray-950 text-white p-2 rounded-md shadow-md text-xs z-10"
      style={{ top: "25px", left: "-400px", minWidth: "50px" }}
    >
      <button 
        onClick={() => handleRetweetAction(post.id, "Retweet")} 
        className="flex items-center space-x-2 w-full text-left hover:bg-gray-700 p-2 rounded-md"
      >
      ...
      <button 
        onClick={() => handleRetweetAction(post.id, "Quote")} 
        className="flex items-center space-x-2 w-full text-left hover:bg-gray-700 p-2 rounded-md mt-1"
      >
        ✏️ <span>Quote</span>
      </button>
    </div>
